...
export default function MainSiteSelector() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F0F2] text-zinc-900">
      {/* Header */}
      <header className="border-b border-zinc-200 bg-white/60 backdrop-blur-xl">
        <div className="max-w-3xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-zinc-900">Welcome, {user?.name}</h1>
              <p className="text-sm text-zinc-500">Select a site to continue</p>
            </div>
            <Button variant="ghost" size="sm" onClick={logout} className="gap-2 text-zinc-500">
              <LogOut className="w-4 h-4" />
              Sign Out
            </Button>
          </div>
              ...
              <div key={envName} data-testid={`env-group-${envName.toLowerCase().replace(/\s+/g, '-')}`}>
                ...
                <div className="space-y-3">
                  {sites.map(site => (
                    <Card
                      key={site.id}
                      className="bg-white/80 backdrop-blur border-zinc-200 hover:border-zinc-300 cursor-pointer transition-all hover:shadow-md"
                      ...
                      <CardContent className="flex items-center justify-between p-4">
                        <div className="flex items-center gap-4">
                          {site.logo_url ? (
                            ...
                            <div className="w-12 h-12 rounded-lg bg-zinc-100 flex items-center justify-center" style={{ borderLeft: `3px solid ${color}` }}>
                              <Globe className="w-6 h-6 text-zinc-400" />
                            </div>
                          )}
                          <div>
                            <h3 className="font-semibold text-lg text-zinc-900">{site.name}</h3>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-sm text-zinc-400">/{site.slug}</span>
