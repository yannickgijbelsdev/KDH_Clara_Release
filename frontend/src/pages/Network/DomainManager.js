      ...
      {activeTab === 'overview' && overview && (
        <div className="space-y-6">
          <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
            {[
              { label: 'CONFIGURED', value: overview.configured_domains, extra: `/ ${overview.total_sites}`, icon: Link2, color: '#10b981', img: '/images/env_domain.jpg' },
              { label: 'KOODH.COM', value: overview.koodh_domains, icon: Globe, color: '#3b82f6', img: '/images/env_domain.jpg' },
              { label: 'CUSTOM', value: overview.custom_domains, icon: ExternalLink, color: '#a855f7', img: '/images/env_domain.jpg' },
              { label: 'VERIFIED', value: overview.verified, icon: CheckCircle, color: '#22c55e', img: '/images/env_domain.jpg' },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
          ...
          {overview.unconfigured > 0 && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-amber-700 font-medium">{overview.unconfigured} site{overview.unconfigured !== 1 ? 's' : ''} without domain configuration</p>
                <p className="text-xs text-amber-500">Go to "Site Domains" to assign domains</p>
              </div>
              <Button size="sm" variant="outline" className="ml-auto" onClick={() => setActiveTab('sites')}>Configure</Button>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center gap-2 mb-3 px-1">
              ...
              <span className="text-sm font-medium text-zinc-600">Active Subdomain Routes</span>
            </div>
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
              ...
              {routes.filter(r => r.is_active).length === 0 && (
                <p className="text-sm text-zinc-400 text-center py-4 w-full">No active routes</p>
              )}
            </div>
          </div>
        </div>
          ...
          {configs.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3 px-1">
                ...
                <span className="text-sm font-medium text-emerald-600">Configured Domains ({configs.length})</span>
              </div>
              <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
                  ...
                  return (
                    ...
                    >
                      <div className="rounded-2xl overflow-hidden border border-black/[0.06] shadow-[0_4px_24px_rgba(0,0,0,0.06)] group-hover:shadow-[0_8px_32px_rgba(0,0,0,0.10)] group-hover:scale-[1.02] transition-all duration-300" style={{ background: 'linear-gradient(160deg, #ffffff 0%, #f9f8f6 100%)' }}>
                        ...
                        <div className="h-1" style={{ background: `linear-gradient(90deg, ${domainColor}, ${domainColor}60)` }} />
                      </div>
                    </motion.div>
                  );
                })}
          ...
          <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
            {routes.map((route, i) => {
              ...
              >
                <div className={`rounded-2xl overflow-hidden border shadow-[0_4px_24px_rgba(0,0,0,0.06)] group-hover:shadow-[0_8px_32px_rgba(0,0,0,0.10)] group-hover:scale-[1.02] transition-all duration-300 ${route.is_active ? 'border-black/[0.06]' : 'border-zinc-200 opacity-60'}`} style={{ background: 'linear-gradient(160deg, #ffffff 0%, #f9f8f6 100%)' }}>
                  ...
                  <div className="px-3.5 py-3">
                    <h3 className="text-sm font-bold font-mono text-zinc-800 truncate">{route.subdomain}.{baseDomain}</h3>
                    <p className="text-[11px] text-zinc-400 truncate mt-0.5">{route.label}{route.description ? ` — ${route.description}` : ''}</p>
                    <div className="flex items-center gap-1 mt-1 text-[11px] text-zinc-400">
                      <ArrowRight className="w-3 h-3" />
                      <span className="font-mono truncate">{route.target_path}</span>
                    </div>
                  ...
                  <div className="h-1" style={{ background: `linear-gradient(90deg, ${routeColor}, ${routeColor}60)` }} />
                </div>
              </motion.div>
              );
            })}
            ...
            >
              <button onClick={openCreateRoute} className="w-full rounded-2xl overflow-hidden border-2 border-dashed border-zinc-200 hover:border-indigo-300 transition-colors h-full min-h-[300px] flex flex-col items-center justify-center gap-3 group" data-testid="create-route-card">
                ...
                <span className="text-sm text-zinc-400 group-hover:text-indigo-500 font-medium transition-colors">New Route</span>
              </button>
            </motion.div>
          </div>
          {routes.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16">
              <ArrowRight className="w-12 h-12 text-zinc-300 mb-3" />
              <p className="text-zinc-400 text-sm">No subdomain routes configured yet</p>
            </div>
          )}
        </div>
      )}
