...
import {
  ...
  ChevronLeft, Copy, Check, Eye, EyeOff, Terminal, Shield, X
} from 'lucide-react';
  ...
  return (
    <div className="h-full flex bg-zinc-950" data-testid="enterprise-chat-view">
      ...
      {showPreview !== null && (() => {
        ...
        if (!code) return null;
        const watermarkHtml = code.replace(
          '</body>',
          `<div style="position:fixed;bottom:12px;right:12px;background:rgba(0,0,0,0.75);color:#a78bfa;font-size:10px;padding:4px 10px;border-radius:20px;font-family:system-ui;backdrop-filter:blur(8px);border:1px solid rgba(167,139,250,0.3);z-index:9999;">Generated by Clara Enterprise Code Assistant</div></body>`
        );
        return (
          <>
            ...
            {/* Panel */}
            <div
              className="fixed top-0 left-0 bottom-0 z-50 w-[600px] max-w-[85vw] bg-zinc-950 border-r border-zinc-800 shadow-2xl flex flex-col animate-[slide-in-left_0.3s_ease-out]"
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-500">Clara Enterprise Code Assistant</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowPreview(null)}
                  className="w-7 h-7 rounded-lg hover:bg-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex border-b border-zinc-800">
                <button
                  ...
                  className={`flex-1 px-4 py-2.5 text-xs font-medium transition-colors ${previewTab === 'preview' ? 'text-violet-400 border-b-2 border-violet-500' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                  <Eye className="w-3 h-3 inline mr-1.5" />Live Preview
                </button>
                <button
                  onClick={() => setPreviewTab('code')}
                  className={`flex-1 px-4 py-2.5 text-xs font-medium transition-colors ${previewTab === 'code' ? 'text-violet-400 border-b-2 border-violet-500' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                ...
                {previewTab === 'preview' ? (
                  <div className="h-full flex flex-col">
                    ...
                    <div className="flex items-center gap-2 px-3 py-2 bg-zinc-900 border-b border-zinc-800">
                      ...
                      <div className="flex-1 bg-zinc-800 rounded-md px-3 py-1">
                        <span className="text-[10px] text-zinc-500 font-mono">preview://localhost</span>
                      </div>
                    </div>
                    <iframe
                      srcDoc={watermarkHtml}
                      title="Code Preview"
                  ...
                  <div className="h-full overflow-auto p-4">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">HTML</span>
                      <button
                        onClick={() => { navigator.clipboard.writeText(code); setCopied('panel'); setTimeout(() => setCopied(null), 2000); }}
                        className="flex items-center gap-1 px-2 py-1 rounded-md bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white text-[11px] transition-colors"
                      >
                        {copied === 'panel' ? <Check className="w-3 h-3 text-green-400" /> : <Copy className="w-3 h-3" />}
                        {copied === 'panel' ? 'Copied' : 'Copy'}
                      </button>
                    </div>
                    <pre className="text-xs text-zinc-300 font-mono leading-relaxed whitespace-pre-wrap break-all bg-zinc-900 rounded-xl p-4 border border-zinc-800">
                      <code>{code}</code>
                    </pre>
                    {/* Watermark */}
